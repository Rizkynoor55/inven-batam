...
/**
 * Generated class for the DataKibCPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-data-kib-c',
  templateUrl: 'data-kib-c.html',
})
export class DataKibCPage {
  items : any = [];
...
load(){
   ...
   loader.present().then(() => {
   this.http.get('http://localhost/api/api.php?command=TampilKibC&kd_lokasi='+this.kode_lokasi)
   .timeout(15000)
   .map(res => res.json())
  .subscribe((success)=>{
...
CariAset(event) {
       
   ...
   if (val && val.trim() != "") {
       ...
       item.luas_lantai.toLowerCase().indexOf(val.toLowerCase()) > -1 || item.asal_usul.toLowerCase().indexOf(val.toLowerCase()) > -1
);
 ...
 tambah_kib_c(param)
 {
    this.navCtrl.push('TambahKibCPage',param);
 }
   ...
   let alert = this.alertCtrl.create({
      ...
      buttons: [
          ...
          },
          {
              ...
              handler: () => {
               loader.present().then(() => {
                  this.http.get('http://192.168.1.1/api/api.php?command=RbKibC&kd_lokasi='+kode_lokasi+'&id_aset='+id_aset)
                  .timeout(15000)
                  .subscribe((success)=>{
                  loader.dismiss();
                  const toast = this.toastCtrl.create({
   ...
   let alert = this.alertCtrl.create({
      ...
      buttons: [
          ...
          },
          {
              ...
              handler: () => {
               loader.present().then(() => {
                  this.http.get('http://192.168.1.1/api/api.php?command=BKibC&kd_lokasi='+kode_lokasi+'&id_aset='+id_aset)
                  .timeout(15000)
                  .subscribe((success)=>{
                  loader.dismiss();
                  const toast = this.toastCtrl.create({
   ...
   let alert = this.alertCtrl.create({
      ...
      buttons: [
          ...
          },
          {
              ...
              handler: () => {
               loader.present().then(() => {
                  this.http.get('http://192.168.1.1/api/api.php?command=VerifKibC&kd_lokasi='+kode_lokasi+'&id_aset='+id_aset+'&verifikator='+this.data_user.nama+'&tgl_verifikasi='+this.Tanggal)
                  .timeout(15000)
                  .subscribe((success)=>{
                  loader.dismiss();
                  const toast = this.toastCtrl.create({
 ...
 BatalVerifKibC(id_aset,kode_lokasi)
 {
   let alert = this.alertCtrl.create({
      ...
      buttons: [
          ...
          },
          {
              ...
              handler: () => {
               loader.present().then(() => {
                  this.http.get('http://192.168.1.1/api/api.php?command=BatalVerifKibC&kd_lokasi='+kode_lokasi+'&id_aset='+id_aset+'&verifikator='+this.data_user.nama+'&tgl_verifikasi='+this.Tanggal)
                  .timeout(15000)
                  .subscribe((success)=>{
                  loader.dismiss();
                  const toast = this.toastCtrl.create({
