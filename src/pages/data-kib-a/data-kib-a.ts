...
/**
 * Generated class for the DataKibAPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-data-kib-a',
  templateUrl: 'data-kib-a.html',
})
export class DataKibAPage {
  items : any = [];
...
load(){
   ...
   loader.present().then(() => {
   this.http.get('http://localhost/api/api.php?command=TampilKibA&kd_lokasi='+this.kode_lokasi)
   .timeout(15000)
   .map(res => res.json())
  .subscribe((success)=>{
...
CariAset(event) {
 ...
 tambah_kib_a(param)
 {
    this.navCtrl.push('TambahKibAPage',param);
 }


 VerifKibA(id_aset,kode_lokasi)
 {
   let alert = this.alertCtrl.create({
      ...
      buttons: [
          ...
          },
          {
              ...
              handler: () => {
               loader.present().then(() => {
                  this.http.get('http://192.168.1.1/api/api.php?command=VerifKibA&kd_lokasi='+kode_lokasi+'&id_aset='+id_aset+'&verifikator='+this.data_user.nama+'&tgl_verifikasi='+this.Tanggal)
                  .timeout(15000)
                  .subscribe((success)=>{
                  loader.dismiss();
                  const toast = this.toastCtrl.create({
 ...
 BatalVerifKibA(id_aset,kode_lokasi)
 {
   let alert = this.alertCtrl.create({
      ...
      buttons: [
          ...
          },
          {
              ...
              handler: () => {
               loader.present().then(() => {
                  this.http.get('http://192.168.1.1/api/api.php?command=BatalVerifKibA&kd_lokasi='+kode_lokasi+'&id_aset='+id_aset+'&verifikator='+this.data_user.nama+'&tgl_verifikasi='+this.Tanggal)
                  .timeout(15000)
                  .subscribe((success)=>{
                  loader.dismiss();
                  const toast = this.toastCtrl.create({
